...
public class ProductSaveHelper {
	...
	static void setNewExtraImageNames(MultipartFile[] extraImageMultiparts, Product product) {
		if (extraImageMultiparts.length > 0) { // if we get new extra-image files ... then update the file(s)
			for (MultipartFile multipartFile : extraImageMultiparts) {
	...
	static void setMainImageName(MultipartFile mainImageMultipart, Product product) {
		if (!mainImageMultipart.isEmpty()) { // if we get new main-image file ... then update the file 
			String fileName = StringUtils.cleanPath(mainImageMultipart.getOriginalFilename());
